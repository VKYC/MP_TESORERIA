...
class PayrollPaymentLine(models.Model):
    ...
    mp_flujo_id = fields.Many2one(comodel_name="mp.flujo", related='move_id.mp_flujo_id', store=True, readonly=False,
                                  domain="[('grupo_flujo_ids', 'in', mp_grupo_flujo_id)]")
    ...
    mp_grupo_flujo_id = fields.Many2one(comodel_name="mp.grupo.flujo", related='move_id.mp_grupo_flujo_id', store=True,
    ...
    def _compute_amount_total(self):
        for line_id in self:
            if not line_id.move_id:
                line_id.amount_total = 0
            else:
                if self.env.company.currency_id != line_id.currency_id:
                    amount_base_currency = self.env["res.currency"]._get_conversion_rate(
                        line_id.currency_id,
                        self.env.company.currency_id,
                        self.env.company,
                        line_id.payroll_payment_id.date,
                    )
                    line_id.amount_total = amount_base_currency
                else:
                    line_id.amount_total = line_id.move_id.amount_total
    ...
    def unlink(self):
        for record in self:
            if not self.user_has_groups(
                    'base.group_system') and record.payroll_payment_id and record.payroll_payment_id.state != 'draft':
                raise ValidationError(
                    _('No se puede eliminar una factura que ya ha sido enviada a menos que seas administrador.'))
